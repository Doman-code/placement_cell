...
export class AppliedjobComponent implements OnInit {
  ...
  applicants: any;
  application_id: any;
  displayedColumns: string[] = ['Student_Application_Id', 'Company_Name','Post_Name', 'Application_Submission_Date', 'Status'];
  ...
  getTableData() {
    ...
    this._cs.getData2(`/appliedjobs/${this.AS.currentUser.id}`).subscribe(res => {
      console.log(res);
      this.userData = res;
